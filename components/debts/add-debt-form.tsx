...
import type { Debt, DebtGroup } from '@/lib/data'

const groupOptions: { value: DebtGroup; label: string }[] = [
...
export function AddDebtForm({ debt }: { debt?: Debt }) {
  const router = useRouter()
  const isEditing = !!debt
  const [name, setName] = useState(debt?.name ?? '')
  const [group, setGroup] = useState<DebtGroup>(debt?.group ?? 'other')
  const [balance, setBalance] = useState(debt ? String(debt.balance) : '')
  const [interestRate, setInterestRate] = useState(debt ? String(debt.interestRate) : '')
  const [submitting, setSubmitting] = useState(false)
  const [deleting, setDeleting] = useState(false)
  const [error, setError] = useState('')

  const valid = name.trim() && Number(balance) >= 0 && Number(interestRate) >= 0
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true)
    setError('')
    const payload = { name: name.trim(), group, balance: Number(balance), interestRate: Number(interestRate) }
    try {
      if (isEditing) {
        await updateDoc(doc(db, 'households', HOUSEHOLD_ID, 'debts', debt.id), payload)
      } else {
        await addDoc(collection(db, 'households', HOUSEHOLD_ID, 'debts'), payload)
      }
      router.push('/debts')
    } catch (err) {
      console.error('Save debt error:', err)
      setError('Could not save this debt. Try again.')
      setSubmitting(false)
    }
  }
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true)
    try {
      await deleteDoc(doc(db, 'households', HOUSEHOLD_ID, 'debts', debt.id))
      router.push('/debts')
    } catch (err) {
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-6">
      <div className="flex flex-col gap-1.5">
        ...
        <input
          id="name"
          type="text"
          placeholder="e.g. Car Loan, Chase Credit Card"
          value={name}
      ...
      <div className="flex flex-col gap-3 sm:flex-row">
        ...
        >
          {submitting ? 'Saving...' : isEditing ? 'Save changes' : 'Save debt'}
        </button>
        {isEditing && (
          <button
            type="button"
